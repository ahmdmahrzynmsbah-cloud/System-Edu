...
export default function StudentsList({ userRole }: StudentsListProps = {}) {
  ...
  const confirmArchiveStudent = () => {
    if (studentToArchive) {
      ...
      setSuccessMessage(`تم أرشفة الطالب "${studentToArchive.name}" بنجاح`);
  ...
  const confirmRestoreStudent = () => {
    if (studentToRestore) {
      ...
      setSuccessMessage(`تم إعادة تفعيل الطالب "${studentToRestore.name}" بنجاح`);
                    ...
                    >
                      ...
                      <td className="px-4 py-3 pl-6">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {student.is_archived ? (
                            <button onClick={() => setStudentToRestore(student)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="إعادة تفعيل الطالب">
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between gap-1.5 pt-2 border-t border-slate-100/70">
                  <div className="flex items-center gap-1">
                    ...
                    {student.is_archived ? (
                      <button onClick={() => setStudentToRestore(student)} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg border border-slate-150 bg-slate-50/50 cursor-pointer" title="إعادة تفعيل الطالب">
                        <ArchiveRestore className="w-4 h-4" />
                      </button>
                    ) : (
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 text-emerald-600 mb-4">
                ...
                <h3 className="text-xl font-bold">إعادة تفعيل الطالب</h3>
              </div>
              <p className="text-slate-600 mb-6 leading-relaxed">
                هل أنت متأكد من رغبتك في إعادة تفعيل الطالب <span className="font-bold text-slate-800">"{studentToRestore.name}"</span>؟ سيعود للقوائم النشطة فوراً.
              </p>
              <div className="flex items-center justify-end gap-3">
                <button
