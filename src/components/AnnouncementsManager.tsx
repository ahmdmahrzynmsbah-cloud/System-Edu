...
export default function AnnouncementsManager() {
  ...
  const handleOpenEdit = (ann: Announcement) => {
    ...
    setFormData({
      ...
      display_style: ann.display_style || 'card',
      action_text: ann.action_text || '',
      action_link: ann.action_link || '',
      icon_type: ann.icon_type || 'megaphone',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      const updated: Omit<Announcement, 'publish_date' | 'views_count'> & { publish_date?: string, views_count?: number } = {
        ...
        display_style: formData.display_style || 'card',
        action_text: formData.action_text?.trim() || undefined,
        action_link: formData.action_link?.trim() || undefined,
        icon_type: formData.icon_type || 'megaphone',
        ...
        {sortedAnnouncements.length === 0 ? (
          ...
          sortedAnnouncements.map((ann) => {
            ...
            const IconComponent = {
              ...
              star: <Star className="w-4 h-4 text-sky-500" />
            }[ann.icon_type || 'megaphone'] || <Megaphone className="w-4 h-4" />;

            const displayStyleLabel = {
              card: { text: 'كارت جداري', style: 'bg-slate-100 text-slate-700' },
              marquee_banner: { text: 'شريط متحرك', style: 'bg-rose-50 text-rose-700 border border-rose-100' },
              popup_modal: { text: 'نافذة منبثقة', style: 'bg-purple-50 text-purple-700 border border-purple-100' }
            ...
            return (
              <motion.div
                ...
                className={`p-5 rounded-2xl border ${currentTheme.bg} transition-all relative flex flex-col justify-between shadow-2xs group min-h-[250px] ${ann.is_pinned ? 'ring-2 ring-[#0D5C8C]' : ''}`}
              >
                <div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-3.5">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${displayStyleLabel.style}`}>
                        {displayStyleLabel.text}
                      </span>
                    </div>
                  ...
                  {ann.image_url && (
                    <div className="w-full h-24 rounded-lg overflow-hidden mb-3 border border-slate-200 bg-slate-50 relative">
                      <img 
                        src={ann.image_url} 
                        alt={ann.title} 
                  ...
                  <div className="flex items-start gap-2 mb-2">
                    <span className="p-1 bg-white rounded-lg border border-slate-200 shrink-0 text-slate-700">
                      {IconComponent}
                    </span>
                    <h3 className="font-extrabold text-slate-800 text-sm leading-snug group-hover:text-[#0D5C8C] transition-colors">
                      {ann.title}
                    </h3>
                  </div>

                  <p className="text-xs text-slate-600 leading-relaxed font-sans line-clamp-4 whitespace-pre-line mb-3">
                    {ann.content}
                  </p>

                  {/* Action link indicator */}
                  {ann.action_text && (
                    <div className="mb-4 inline-flex items-center gap-1 text-[10px] bg-white border border-slate-150 px-2.5 py-1 rounded-lg font-bold text-slate-700">
                      <MousePointerClick className="w-3 h-3 text-sky-600" />
                      <span>زر تفاعلي: </span>
                      <span className="text-sky-700 font-extrabold">{ann.action_text}</span>
                    </div>
                  )}
                </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* 🚀 New Advanced Advertisement Layout Options */}
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-150 space-y-4">
                <h4 className="text-xs font-extrabold text-[#0D5C8C] flex items-center gap-1">
                  <Sparkles className="w-3.5 h-3.5" />
                  خيارات العرض والتصميم الإعلاني الاحترافي
                </h4>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Display Style */}
                  <div className="space-y-1.5">
                    <label className="block text-xxs font-extrabold text-slate-600">طريقة العرض والظهور</label>
                    <select
                      value={formData.display_style}
                      onChange={(e) => setFormData({ ...formData, display_style: e.target.value as any })}
                      className="w-full text-xs font-sans border border-slate-200 p-2.5 rounded-xl text-slate-700 bg-white cursor-pointer focus:ring-2 focus:ring-[#0D5C8C]/25"
                    >
                      <option value="card">كارت جداري عادي في اللوحة</option>
                      <option value="marquee_banner">شريط إعلاني متحرك (Ticker) أعلى لوحة التحكم</option>
                      <option value="popup_modal">نافذة منبثقة تفاعلية مميزة (Popup Modal) فور الدخول</option>
                    </select>
                  </div>

                  {/* Icon Type */}
                  <div className="space-y-1.5">
                    <label className="block text-xxs font-extrabold text-slate-600">أيقونة الإعلان المرافقة</label>
                    <select
                      value={formData.icon_type}
                      onChange={(e) => setFormData({ ...formData, icon_type: e.target.value as any })}
                      className="w-full text-xs font-sans border border-slate-200 p-2.5 rounded-xl text-slate-700 bg-white cursor-pointer focus:ring-2 focus:ring-[#0D5C8C]/25"
                    >
                      <option value="megaphone">📢 بوق إعلامي وتوجيهات عامة</option>
                      <option value="trophy">🏆 كأس تفوق، تكريم ودرجات متميزة</option>
                      <option value="gift">🎁 عرض ترويجي، تخفيض أو هدية</option>
                      <option value="alert">⚠️ تنبيه هام أو قرار إداري عاجل</option>
                      <option value="star">⭐ نجمة تميز، طالب مثالي، أو إعلان خاص</option>
                    </select>
                  </div>
                </div>

                {/* Image URL banner link */}
                <div className="space-y-1.5">
                  <label className="block text-xxs font-extrabold text-slate-600">رابط صورة أو بانر الإعلان الترويجي (اختياري)</label>
                  <div className="relative">
                    <input 
                      ...
                      className="w-full text-xs font-sans border border-slate-200 pl-3 pr-8 py-2 rounded-xl text-slate-700 focus:ring-2 focus:ring-[#0D5C8C]/25 outline-none text-left"
                    />
                    <Image className="w-3.5 h-3.5 text-slate-400 absolute right-3 top-3" />
                  </div>
                  <p className="text-[10px] text-slate-400">يجعل الإعلان يبدو كبانر تسويقي حقيقي بوضع صورة جذابة للمكافآت أو المذكرات.</p>
                </div>

                {/* CTA Action button & link */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="block text-xxs font-extrabold text-slate-600">نص الزر التفاعلي (CTA) (اختياري)</label>
                    <input 
                      type="text"
                      value={formData.action_text}
                      onChange={(e) => setFormData({ ...formData, action_text: e.target.value })}
                      placeholder="مثال: سجل الآن، تفاصيل الكورس"
                      className="w-full text-xs font-sans border border-slate-200 p-2 py-2 rounded-xl text-slate-700 focus:ring-2 focus:ring-[#0D5C8C]/25 outline-none"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="block text-xxs font-extrabold text-slate-600">رابط الزر أو التوجيه (اختياري)</label>
                    <input 
                      type="text"
                      value={formData.action_link}
                      onChange={(e) => setFormData({ ...formData, action_link: e.target.value })}
                      placeholder="https://... أو اسم التبويب"
                      className="w-full text-xs font-sans border border-slate-200 p-2 py-2 rounded-xl text-slate-700 focus:ring-2 focus:ring-[#0D5C8C]/25 outline-none text-left"
                    />
                  </div>
                </div>
              </div>
